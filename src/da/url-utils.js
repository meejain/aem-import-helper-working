 ...
 * Modified to include directory structure to avoid filename conflicts.
 * @param {string} url - The URL to extract and sanitize the filename from
 * @return {string} Sanitized filename with preserved extension and directory info
 */
export function getSanitizedFilenameFromUrl(url) {
  ...
  // Extract directory path and create unique identifier
  const dirPath = path.dirname(pathname);
  const dirParts = dirPath.split('/').filter(part => part && part !== '.' && part !== 'users_uploads');
  
  // Find the generated folder number (e.g., "generated/300", "generated/1242")
  const generatedIndex = dirParts.findIndex(part => part === 'generated');
  let uniqueId = '';
  
  if (generatedIndex !== -1 && generatedIndex < dirParts.length - 1) {
    // Use the number after "generated" as unique identifier
    uniqueId = dirParts[generatedIndex + 1];
  }
  ...
  // Create unique filename: originalname-uniqueid.ext (for all files to ensure uniqueness)
  let finalBase = base;
  if (uniqueId) {
    finalBase = `${base}-${uniqueId}`;
  }
  
  const sanitizedBase = sanitizeFilename(finalBase);

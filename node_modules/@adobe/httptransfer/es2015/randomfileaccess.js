...
function _openExtendFile() {
  _openExtendFile = _asyncToGenerator(function* (path, flags, size = 0) {
    logger.info(`Opening file ${path}, extend size is ${size} (with flags ${flags})`);
...
function _getOrOpen() {
  _getOrOpen = _asyncToGenerator(function* (fileHandles, path, flags, totalLength) {
    ...
    if (!fileHandlePromise) {
      logger.info(`Opening file ${path} with flags ${flags} and total write-only length ${totalLength}`);
      fileHandlePromise = openExtendFile(path, flags, totalLength);
      fileHandles.set(path, fileHandlePromise);
    } else {
      logger.info(`Reusing existing filehandle promise for file ${path} (requested flags ${flags} and total write-only length ${totalLength})`);
    }
...
class RandomFileAccess {
  ...
  close(path) {
    ...
    return _asyncToGenerator(function* () {
      if (path) {
        ...
        if (fileHandlePromise) {
          logger.info(`Closing file handle for ${path}`);
  ...
  write(path, range, buffer, totalSize) {
    ...
    return _asyncToGenerator(function* () {
      ...
      logger.info(`Writing buffer of size ${buffer.length} to file`);
  ...
  read(path, range) {
    ...
    return _asyncToGenerator(function* () {
      ...
      logger.info(`Getting file handle for reading file ${path}`);
      const handle = yield getOrOpen(fileHandles, path, "r");
      const buffer = Buffer.allocUnsafe(range.length);
      logger.info(`Allocating ${range.length} for reading from file ${path}`);
      const {
        bytesRead
      } = yield handle.read(buffer, 0, range.length, range.low);
      logger.info(`Read ${bytesRead} from file ${path}`);
